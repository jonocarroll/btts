...
no_htmlify <- function() {
  ...
  R2Hframe <- grep("^Rd2HTML", sapply(sys.calls(), function(a)
...
html_file <- function(filename) {
  no_htmlify()
  return(paste(readLines(system.file("doc", filename, package = packageName())),
               collapse = "\n"))
}
...
change_stylesheet <- function(sheet) {
  R2Hframe <- grep("^Rd2HTML", sapply(sys.calls(), function(a)
      paste(deparse(a), collapse = "\n")))
  if (length(R2Hframe) != 0) {
    assign("stylesheet", file.path("..", "doc", sheet),
           envir = sys.frame(which = max(R2Hframe)))
  }
}
